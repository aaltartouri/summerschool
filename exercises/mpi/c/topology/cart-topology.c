...
int main(int argc, char* argv[]) {
    int ntasks, my_id, irank, ndims = 2;
    ...
    /* TO DO */
    MPI_Cart_create(MPI_COMM_WORLD, ndims, dims, period, 0, &comm2d);
    ...
    /* TO DO */
    MPI_Cart_coords(comm2d, my_id, ndims, coords);

    for (irank = 0; irank < ntasks; irank++) {
        if (my_id == irank) { // to insure things are pronted out nicely!
